...
extension Reactive where Base: NSObject, Base: ActionMessageSending {
	internal var proxy: ActionProxy<Base> {
		...
		return base.synchronized {
			...
			// Invoke the original target and action setters to start the
			// interception.
			//
			// `instance.objcClass` (aka `+class`) would report the perceived
			// class instead of the real runtime subclass generated by RAC or
			// KVO.
			let realClass: AnyClass = base.objcClass
			let originalTargetSetterImpl = class_getMethodImplementation(realClass, #selector(setter: base.target))!
			let originalActionSetterImpl = class_getMethodImplementation(realClass, #selector(setter: base.action))!
			
			typealias CTargetSetter = @convention(c) (NSObject, Selector, AnyObject?) -> Void
			typealias CActionSetter = @convention(c) (NSObject, Selector, Selector?) -> Void
			
			unsafeBitCast(originalTargetSetterImpl, to: CTargetSetter.self)(base,  #selector(setter: base.target), proxy)
			unsafeBitCast(originalActionSetterImpl, to: CActionSetter.self)(base,  #selector(setter: base.action), #selector(proxy.invoke(_:)))
